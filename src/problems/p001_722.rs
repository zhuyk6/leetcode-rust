...
mod tests{
    ...
    fn example() {
        ...
        assert_eq!(1, Solution::minimum_hamming_distance(source, target, allowedSwaps));

        let source = vec![1,2,3,4];
        let target = vec![1,3,2,4];
        let allowedSwaps = vec![];
        assert_eq!(2, Solution::minimum_hamming_distance(source, target, allowedSwaps));

        let source = vec![5,1,2,4,3];
        let target = vec![1,5,4,2,3];
        let allowedSwaps = vec![vec![0,4],vec![4,2], vec![1,3], vec![1,4]];
        assert_eq!(0, Solution::minimum_hamming_distance(source, target, allowedSwaps));
    }
    ...
    fn test1() {
        ...
        let allowedSwaps = vec![vec![0,2],vec![1,2]];
        assert_eq!(1, Solution::minimum_hamming_distance(source, target, allowedSwaps));
    }
}
