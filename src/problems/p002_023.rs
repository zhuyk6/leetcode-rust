...
impl Node {
    ...
    pub fn search<I>(&self, iter: I, v: &mut Vec<usize>)
    where
...
pub fn num_of_pairs(nums: Vec<String>, target: String) -> i32 {
    ...
    for i in 0..n {
        for j in 0..n {
            if i == j || st[i] == usize::MAX || ed[j] == usize::MAX {continue;}
            if st[i] + 1 == ed[j] {
                println!("(i, j) = ({i}, {j})");
                ans += 1;
            }
...
#[test]
fn example1() {
    let nums = vec!["777".to_string(), "7".to_string(), "77".to_string(), "77".to_string()];
...
#[test]
fn example2() {
    let nums = vec!["123".to_string(), "4".to_string(), "12".to_string(), "34".to_string()];
